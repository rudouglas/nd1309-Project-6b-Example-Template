...
{
    // Define 'owner'
    // address owner; // Remove as Ownable is inherited
    ...
    event Fertilised(uint256 upc);
    event Dead(uint256 upc);
    ...
    // No longer needed as modifier is inherited from the Ownable contract
    // modifier onlyOwner() {
    ...
    modifier fertilised(uint256 _upc) {
        require(items[_upc].itemState == State.Fertilised);
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Processed
    modifier notDead(uint256 _upc) {
        require(items[_upc].itemState != State.Dead);
        _;
    }
    ...
    // Define a function 'FertiliseItem' that allows a farmer to mark an item 'Harvested'
    function fertiliseItem(uint256 _upc, address _originFarmerID)
        public
        onlyFarmer
    {
        // Add the new item as part of Fertilise
        items[_upc].upc = _upc;
        items[_upc].ownerID = _originFarmerID;

        items[_upc].itemState = State.Fertilised;

        // Emit the appropriate event
        emit Fertilised(_upc);
    }

    // Define a function 'killItem' that allows a farmer to mark an item 'Dead'
    function killItem(uint256 _upc, string _productNotes)
        public
        verifyCaller(items[_upc].ownerID)
    {
        // Add the new item as part of Fertilise
        items[_upc].productNotes = _productNotes;
        items[_upc].itemState = State.Dead;
    ...
    ) public notDead(_upc) fertilised(_upc) verifyCaller(items[_upc].ownerID) {
    ...
    function packageItem(uint256 _upc)
        public
        harvested(_upc)
